...
function SupportTicketsTeaser() {
  const { data, isLoading } = useSWR(
    ['platform-tickets', 'open'],
  ...
  return (
    <Link
      href="/platform/support"
      className="card p-5 flex items-center justify-between gap-4 hover:border-indigo-200 transition-colors"
    >
      <div className="flex items-start gap-3 min-w-0">
        <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center shrink-0">
          <LifeBuoy size={18} className="text-indigo-700" />
        </div>
        <div className="min-w-0">
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {isLoading
              ...
              : openCount === 0
                ...
                : `${openCount} open ticket${openCount === 1 ? '' : 's'} from clients.`}
          </p>
        </div>
      </div>
      <span className="text-sm font-semibold text-indigo-700 shrink-0">Open queue →</span>
    </Link>
  );
}
