...
import { googleAuthUrl, persistReferralCode } from '@/lib/referral';
...
/** Stash `?ref=` from any public URL into cookie + localStorage (first-touch). */
...
function GoogleOAuthLinkInner({
  ...
  const params = useSearchParams();
  const apiBase = process.env.NEXT_PUBLIC_API_URL ?? DEFAULT_API;
  return (
    <a href={googleAuthUrl(apiBase, params.get('ref'))} className={className}>
