...
## 🏥 Project Context: AI Receptionist Dashboard

This project is a SaaS platform for AI-powered phone receptionists targeting dental and medical practices.

### Design System
- **Primary brand color**: Indigo — `brand-600: #4f46e5`, full scale in `tailwind.config.ts`
- **UI framework**: Tailwind CSS 3.4 + Radix UI primitives (unstyled, we style them)
- **Icons**: Lucide React only (no other icon libraries)
- **Font**: System font stack (Tailwind default — Inter on most systems)

### Global Design Tokens (`apps/dashboard/src/app/globals.css`)
```css
.btn-primary   — indigo filled button with hover/focus states
.btn-secondary — white border button
.btn-danger    — red variant
.card          — white rounded shadow container (bg-white, rounded-xl, shadow-sm, border)
.badge         — small pill label (extend with color variants)
.input         — form input with focus ring
```

### Page Contexts
| Area | Background | Feel |
|------|-----------|------|
| Landing page hero | `bg-gray-950` dark | Bold, modern, premium SaaS |
| Landing page sections | `bg-gray-900` / `bg-gray-950` | Dark with indigo accents |
| Dashboard (app) | `bg-gray-50` light | Clean, professional, clinical |
| Auth pages | `bg-gray-50` | Minimal, trustworthy |
| Sidebar | `bg-white` with border | Crisp navigation |

### Brand Voice
- Professional and trustworthy (healthcare context)
- Not overly clinical — warm and approachable
- Tech-forward but not jargon-heavy
- Tagline tone: "Your AI picks up every call."

### Do Not
- Use red/orange as primary (only for errors/danger)
- Mix multiple green shades (emerald = appointments only)
- Add decorative fonts or cursive
- Use shadows heavier than `shadow-md` in the app (landing can go to `shadow-xl`)
