...
11. **Outbound feature gating for promo-trial tenants.** ✅ Superseded (2026-07-02) by the **per-tenant rotating outbound number pool**: campaigns no longer need a tenant-owned dialer number at all. `modules/outbound-pool/` auto-provisions ~3 wholesale local numbers per tenant on first campaign (`purpose='outbound_pool'` on `tenant_phone_numbers`, migration 0034), rotates them least-recently-dialed per call (`outbound-dial.job.ts`), and a 6-hourly sweep grows the pool (cap 15) when volume concentrates. Pool numbers carry NO per-number Stripe fee — outbound minutes bill via `minute_usage` (whose Telnyx-path hook in `media-stream.handler.ts` was also fixed the same day; it previously never fired). Legacy campaigns with a non-null `from_number` keep dialing from that fixed number.
